...
SD_Error SD_Init(void)
{
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_DMA2,ENABLE);//使能PORTC,PORTD时钟
	
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_SDIO,ENABLE);//使能SDIO,DMA2时钟
...
{	  
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
  	SD_DMA_Config((u32*)buf,blksize,DMA_DIR_PeripheralToMemory); 
		...
		SDIO_DMACmd(ENABLE);
 		while(((DMA2->LISR & 0x2000)==RESET)&&(TransferEnd==0)&&(TransferError==SD_OK)&&timeout)timeout--;//等待传输完成 
       ...
       SDIO_CmdInitStructure.SDIO_Argument =  addr;//发送CMD18+从addr地址出读取数据,短响应 
 		...
 		}else if(DeviceMode==SD_DMA_MODE)
		{
			...
			SDIO->MASK|=(1<<1)|(1<<3)|(1<<8)|(1<<5)|(1<<9);	//配置需要的中断 
	 		...
	 		while(((DMA2->LISR & 0x2000)==RESET)&&timeout)timeout--;//等待传输完成 
			if(timeout==0)return SD_DATA_TIMEOUT;//超时
...
{		 
	...
	DMA_DeInit(DMA2_Stream3);   //将DMA2的通道4寄存器重设为缺省值
	DMA_Cmd(DMA2_Stream3, DISABLE ); //关闭DMA2 通道4

	DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&SDIO->FIFO;  //DMA外设基地址
	DMA_InitStructure.DMA_Channel = DMA_Channel_4;
	DMA_InitStructure.DMA_Memory0BaseAddr = (u32)mbuf;  //DMA内存基地址
...
//  DMA_InitStructure.DMA_MemoryBurst = DMA_MemoryBurst_4;  // 内存突发传输
//  DMA_InitStructure.DMA_PeripheralBurst = DMA_PeripheralBurst_4;  // 外设突发传输
  //DMA通道x没有设置为内存到内存传输
	DMA_Init(DMA2_Stream3, &DMA_InitStructure);  //根据DMA_InitStruct中指定的参数初始化DMA的通道USART1_Tx_DMA_Channel所标识的寄存器

	DMA_Cmd(DMA2_Stream3, ENABLE ); //开启DMA2 通道4
}   
//读SD卡
//buf:读数据缓存区
//sector:扇区地址
