...
#define DEBUG_VS1053
#ifdef DEBUG_VS1053
#define  VS1053_DEBUG(x) UART_printf x
#else 
#define VS1053_DEBUG(x)
#endif
/* VS10XX默认设置参数 */
_vs10xx_obj vsset =
{
...
{
    return spi2_read_write_byte(data);
}
...
{
    spi2_set_speed(SPI_SPEED_32); /* 设置到低速模式 */
}
...
{
    spi2_set_speed(SPI_SPEED_8); /* 设置到高速模式 */
}
...
void vs10xx_init(void)
{
    ...
    GPIO_InitTypeDef GPIO_InitStructure;
		...
		GPIO_InitStructure.GPIO_Pin = VS10XX_RST_GPIO_PIN;
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(VS10XX_RST_GPIO_PORT, &GPIO_InitStructure);
	
		GPIO_InitStructure.GPIO_Pin = VS10XX_XCS_GPIO_PIN;
		GPIO_Init(VS10XX_XCS_GPIO_PORT, &GPIO_InitStructure);

		GPIO_InitStructure.GPIO_Pin = VS10XX_XDCS_GPIO_PIN;
		GPIO_Init(VS10XX_XDCS_GPIO_PORT, &GPIO_InitStructure);

		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
		GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
		GPIO_InitStructure.GPIO_Pin = VS10XX_DQ_GPIO_PIN;
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(VS10XX_DQ_GPIO_PORT, &GPIO_InitStructure);
    ...
    while (vs10xx_read_reg(SPI_CLOCKF) != 0X9800)   /* 设置VS10XX的时钟,3倍频 ,1.5xADD */
    {
        ...
        if (retry++ > 100)
        {
            break;
				...
				VS1053_DEBUG(("SPI_CLOCKF = 0x%X\n",vs10xx_read_reg(SPI_CLOCKF)));
    }
...
uint8_t audio_play_song(char *pname)
{
    ...
    rval = 0;
    fmp3 = (FIL *)pvPortMalloc(sizeof(FIL));    /* 申请内存 */
    databuf = (uint8_t *)pvPortMalloc(4096);    /* 开辟4096字节的内存区域 */
		
		VS1053_DEBUG(("fmp3 = %d\n",fmp3));
		VS1053_DEBUG(("databuf = %d\n",databuf));
