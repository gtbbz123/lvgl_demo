 ...
 * 占用空间 : 13字节.
 */
#define TP_SAVE_ADDR_BASE   FLASH_USER_START_ADDR
...
void tp_save_adjust_data(void)
{
     ...
     */
    Flash_WriteBuffer8(TP_SAVE_ADDR_BASE, p, 12);                /* 保存12个字节数据(xfac,yfac,xc,yc) */
    Flash_WriteByte(TP_SAVE_ADDR_BASE + 12, 0X0A);   /* 保存校准值 */
}
...
uint8_t tp_get_adjust_data(void)
{
     ...
     */
    Flash_ReadBuffer8(TP_SAVE_ADDR_BASE, p, 12);                 /* 读取12字节数据 */
    temp = Flash_ReadByte(TP_SAVE_ADDR_BASE + 12);   /* 读取校准状态标记 */
            ...
            switch (cnt)
            {
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    tp_dev.yc = pxy[4][1];      /* Y轴,物理中心坐标 */
										...
										UART_printf("tp_dev.xfac = %f", tp_dev.xfac);
										UART_printf("tp_dev.yfac = %f", tp_dev.yfac);
										UART_printf("tp_dev.xc = %d", tp_dev.xc);
										UART_printf("tp_dev.yc = %d", tp_dev.yc);
...
uint8_t tp_init(void)
{
    ...
    if (lcddev.id == 0x7789 || lcddev.id == 0X5310)                /* 2.4/3.5寸电容触摸屏 */
    {
        if (gt9xxx_init() == 0)             /* 初始化GT系列触摸屏成功,即当前2.4/3.5寸电容触摸屏 */
        {
            tp_dev.scan = gt9xxx_scan;      /* 扫描函数指向GT1151触摸屏扫描 */
            tp_dev.touchtype |= 0X80;       /* 电容屏 */
            return 0;
        }
    }
    if (lcddev.id == 0X5510 || lcddev.id == 0X7796 || lcddev.id == 0X4342 || lcddev.id == 0X1018  || lcddev.id == 0X4384)  /* 电容触摸屏,3.5寸/4.3寸屏 & gtxx系列电容触摸屏 */
    {
        gt9xxx_init();
        tp_dev.scan = gt9xxx_scan;      /* 扫描函数指向GT1151触摸屏扫描 */
        tp_dev.touchtype |= 0X80;       /* 电容屏 */
